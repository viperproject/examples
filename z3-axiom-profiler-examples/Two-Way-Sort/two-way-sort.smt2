(get-info :version)
; (:version "4.4.0")
; Input file is Two-Way-Sort-Slow.sil
; Started: 2015-10-26 17:07:12
; Silicon.buildVersion: 0.1-SNAPSHOT b5ec6a10556f cvc4-integration 2015/10/26 17:01:09
; ------------------------------------------------------------
; Preamble start
; 
; ; /z3config.smt2
...
(assert (forall (($a1 $Array) ($a2 $Array) ($i Int) ($j Int)) (!
  (=>
    (or (not (= $a1 $a2)) (not (= $i $j)))
    (not (= (loc<Array~Int~Ref> $a1 $i) (loc<Array~Int~Ref> $a2 $j))))
  :pattern ((loc<Array~Int~Ref> $a1 $i) (loc<Array~Int~Ref> $a2 $j))
  :qid |prog.all_diff|)))
...
; [eval] length(a) - 1
; [eval] length(a)
; loop at Two-Way-Sort-Slow.sil,46:3
